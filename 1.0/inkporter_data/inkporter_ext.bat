...
for /f "delims=," %%d in ('inkscape --query-all %svgin% ^| findstr %objID%') do (
	inkscape --export-id=%%d --export-filename=%4\temp-%%d.png --export-dpi=%dpi% %svgin% >nul
	inkporter_data\libwebp\bin\cwebp %4\temp-%%d.png -o %4\%%d.webp
...
echo.
inkporter_data\gs9.52\bin\gswin32c -sDEVICE=pdfwrite -dBATCH -dNOPAUSE -sOutputFile=%namaFile% @%4\list.txt
del *.pdfx
del list.txt
goto end
...
for /f "delims=," %%d in ('inkscape --query-all %svgin% ^| findstr %objID%') do (
	...
	inkscape --export-id=%%d --export-filename=%4\%%d.png --export-dpi=%dpi%  %svgin%
	del %4\temp-%%d.svg
	inkporter_data\7-Zip\7z a -tzip %4\%%d.zip %4\%%d.png %4\%%d.eps
	del %4\%%d.svg
	del %4\%%d.png
	del %4\%%d.eps
